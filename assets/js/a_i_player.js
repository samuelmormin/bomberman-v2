...
var a_i_player = function(posX, posY, index_i, index_j, size)
{
    ...
    this.auto_moves = function()
    {
        ...
        // Go to LEFT
        if((displayed_set.bricks_properties[this.index_i][this.index_j - 1].breakable == null)  && this.direction == 'left')
        {
            console.log('go left');
            if(displayed_set.bricks_properties[this.index_i][this.index_j - 2].breakable == null)
            {
                _this.drop_bomb();
                setTimeout(function()
                {
                    _this.move_left(_this, 2);
                }, 100); 
            }
            else if(displayed_set.bricks_properties[this.index_i][this.index_j - 1].breakable == null)
            {
                _this.move_left(_this, 1);
            }
            
        }
        
        // Go RIGHT
        else if(displayed_set.bricks_properties[ _this.index_i ][ _this.index_j + 1 ].breakable == null && this.direction == 'right')
        {
            console.log('go right');
            if(displayed_set.bricks_properties[ _this.index_i ][ _this.index_j + 2 ].breakable == null)
            {
                this.drop_bomb();
                setTimeout(function()
                {
                    _this.move_right(_this, 2);
                }, 100); 
            }
            else if(displayed_set.bricks_properties[this.index_i][this.index_j - 1].breakable == null && _this.bomb_in_action == false)
            {
                _this.move_right(_this, 1);
            }
        }

        // Go UP
        else if(displayed_set.bricks_properties[_this.index_i - 1][_this.index_j].breakable == null && _this.direction == 'up')
        {
            console.log('go top');
            if(displayed_set.bricks_properties[_this.index_i - 1][_this.index_j].breakable == null)
            {
                ...
                setTimeout(function()
                {
                    _this.move_up(_this, 2);
                }, 100); 
            }
            else if(displayed_set.bricks_properties[this.index_i - 1 ][this.index_j ].breakable == null)
            {
                _this.move_up(_this, 1);
            }
        }
        ...
        new function()
        {
            ...
            if(random == 1)
            {
                ...
                console.log('random ='+_this.direction);
            }
            else if(random == 2)
            {
                _this.direction = 'up';
...
//    }
    ...
    // Move DOWN
    this.move_down = function(current)
    {
        current.posX = displayed_set.bricks_properties[ current.index_i ][ current.index_j + 1 ].posX;
        current.posY = displayed_set.bricks_properties[ current.index_i ][ current.index_j + 1 ].posY;
        current.create_a_i_player.style.transform = 'translateX('+ current.posX +'px) translateY('+ current.posY +'px)';
    }
}
